...
function normalizeMetroTerms(value = []) {
  const values =
    Array.isArray(value)
      ? value
      : [value];
...
async function searchBusinessSubscriptions(options = {}) {
  ...
  if (options.name) {
    ...
    where.push(
      `(LOWER(b.business_name) LIKE LOWER($${values.length - 1}) OR LOWER(b.business_id) LIKE LOWER($${values.length}))`
    );
  }

  if (options.industry) {
    addWhere(
      "LOWER(COALESCE(b.business_category,'')) = LOWER(?)",
      options.industry
    );
  }
  ...
  if (options.plan) {
    addWhere(
      "LOWER(COALESCE(bs.plan,'verified_basic')) = LOWER(?)",
      options.plan
    );
  }
  ...
  if (options.status) {
    addWhere(
      "LOWER(COALESCE(bs.subscription_status, bs.status, 'active')) = LOWER(?)",
      options.status
    );
  }
